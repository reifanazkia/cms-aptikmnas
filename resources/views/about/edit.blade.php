    ...
    <form method="POST" action="{{ route('about.update', $about->id) }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full border rounded-lg px-3 py-2 focus:ring-emerald-500 focus:border-emerald-500">{{ old('description', $about->description) }}</textarea>
        ...
        <div>
            ...
            <input type="file" name="image2" class="w-full border rounded-lg px-3 py-2">
            @if($about->image2)
                <img src="{{ asset('storage/' . $about->image2) }}" class="w-32 mt-2 rounded">
            @endif
            @error('image2') <p class="text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <!-- Tombol -->
        <div class="flex justify-end space-x-2">
            <a href="{{ route('about.index') }}" class="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400">Batal</a>
            <button type="submit" class="px-4 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700">Update</button>
        </div>
    </form>
</div>
@endsection
