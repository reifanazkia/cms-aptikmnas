        ...
        <form action="{{ route('aboutus.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <select name="category_tentangkami_id"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}"
                            {{ old('category_tentangkami_id') == $category->id ? 'selected' : '' }}>
                            {{ $category->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" rows="5"
                          class="mt-1 w-full border rounded-xl px-4 py-2 focus:ring-emerald-500 focus:border-emerald-500" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
