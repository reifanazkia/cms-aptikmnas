    ...
    <form action="{{ route('agenda.update', $agenda->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="event_organizer" value="{{ old('event_organizer', $agenda->event_organizer) }}"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
            </div>
        </div>
        ...
        <div>
            ...
            @if ($agenda->image)
                <div class="mb-3">
                    <img src="{{ asset('agenda/' . $agenda->image) }}" alt="Agenda Image" class="h-32 rounded-lg border">
                </div>
            @endif
