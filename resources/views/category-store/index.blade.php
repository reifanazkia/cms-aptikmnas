@extends('layouts.app', ['title' => 'Kategori Store'])
...
<div class="bg-white p-6 rounded-lg space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        <div class="md:flex md:flex-col">
            <h1 class="text-3xl font-bold text-emerald-700">Kategori Store</h1>
...
<div id="addModal" class="fixed inset-0 flex items-center justify-center bg-black/50 hidden z-50">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
        ...
        <form id="addForm" action="{{ route('category-store.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
...
<script>
    ...
    function openEditModal(id, name) {
        let url = '{{ route('category-store.update', ':id') }}'.replace(':id', id);
    ...
    function confirmDelete(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = '{{ route('category-store.destroy', ':id') }}'.replace(':id', id);
