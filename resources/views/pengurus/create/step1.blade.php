...
<div class="max-w-5xl mx-auto px-4 sm:px-0">
    <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 space-y-6">
        ...
        <!-- Form -->
        <form action="{{ route('pengurus.create.step1.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="category_daftar_id" class="block text-sm font-medium text-gray-700">Daftar Pengurus DPD</label>
                    <select id="category_daftar_id" name="category_daftar_id"
                        class="mt-1 block w-full rounded-lg border py-2 px-3 border-gray-300 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        <option value="">Pilih Pengurus</option>
                        @foreach ($categoryDaftar as $category)
                            <option value="{{ $category->id }}" {{ old('category_daftar_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="category_pengurus_id" name="category_pengurus_id"
                        ...
                        @foreach ($categoryPengurus as $category)
                            <option value="{{ $category->id }}" {{ old('category_pengurus_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
