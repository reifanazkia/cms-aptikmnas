        ...
        <div class="bg-white rounded-2xl shadow-lg border border-emerald-100 p-6 sm:p-10 space-y-8">
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="disusun" id="disusun" value="{{ old('disusun', $product->disusun) }}"
                                   class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"
                                   required>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="bahasa" id="bahasa" value="{{ old('bahasa', $product->bahasa) }}"
                                   class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
                        </div>

                        <div>
                            <label for="notlp" class="block text-sm font-medium text-gray-700 mb-1">No. Telepon</label>
                            <input type="text" name="notlp" id="notlp" value="{{ old('notlp', $product->notlp) }}"
                                   class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="category_store_id" id="category_store_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_store_id', $product->category_store_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Upload Gambar -->
                    <div x-data="imageUploadCreate('{{ $product->image ? asset('storage/' . $product->image) : '' }}')" class="space-y-2">
                        ...
                        <div x-show="!previewUrl" x-on:click="triggerInput" x-on:dragover.prevent="isDrag = true"
                            ...
                            <input type="file" name="image" id="image" accept="image/jpeg,image/png"
                                   x-ref="fileInput" class="hidden" x-on:change="handleFiles($event.target.files)">
                        </div>
