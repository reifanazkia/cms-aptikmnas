...
<div class="max-w-3xl mx-auto p-6 bg-white shadow rounded-lg">
    <h1 class="text-2xl font-bold text-emerald-700 mb-6">Tambah Slider</h1>
    ...
    <form action="{{ route('slider.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title') }}"
                   class="w-full rounded-lg border py-2 px-4 border-gray-300 focus:ring-emerald-500 focus:border-emerald-500">
        </div>
        ...
        <div>
            ...
            <textarea name="subtitle" rows="3"
                      class="w-full rounded-lg border py-2 px-4 border-gray-300 focus:ring-emerald-500 focus:border-emerald-500">{{ old('subtitle') }}</textarea>
        </div>
        ...
        <!-- Youtube ID -->
        <div>
            <label class="block text-sm font-medium text-gray-700">Youtube ID</label>
        ...
        <div class="flex items-center">
            <input type="checkbox" name="display_on_home" value="1" {{ old('display_on_home') ? 'checked' : '' }}
                   class="h-4 w-4 text-emerald-600 border py-2 px-4 border-gray-300 rounded">
            <label class="ml-2 text-sm text-gray-700">Tampilkan di Home</label>
        </div>

        <!-- Submit -->
        <div class="flex justify-end">
            <a href="{{ route('slider.index') }}" class="px-4 py-2 rounded-lg bg-gray-300 text-gray-700 mr-2">Batal</a>
            <button type="submit"
